...
const ADS_LERP = 20
var raildamage = 0
var fview = {"Default": 118, "ADS": 50, "ADSSCOPE": 10, "DASH": 200}
...
@onready var raycast = $Head/Camera3D/hand/Buster/RayCast3D
@onready var Chaospawnhurt = $ChaosPawnHurt
@onready var text_timer = $text_timer
@onready var coll = $CollisionShape3D
@onready var ray = $Head/Camera3D/hand/Buster/RayCast3D
...
@onready var aimcast2 = $Head/Camera3D/hand/Stormcloud/RayCast3D
		...
		if current_weapon == 6:
			if Globals.boltammo > 0:
				...
				if Input.is_action_just_released("fire"):
					...
					$Head/Shakeable_camera.add_trauma(0.8)
...
func fire():
	...
	if Input.is_action_pressed("fire") and Globals.upgrade == true:
		if current_weapon == 1:
			...
			$Head/Camera3D/hand/Buster/muzzleflash.show()
			$firesound.play(0.001)
			anim_player.play("XBusterFire", -1, 100)
...
func dash(delta):
	if Input.is_action_pressed("dash") and Input.is_action_pressed("move_forward"):
		speed += 0.1
		dashing = true
		anim_player.play("Weaponsway",-1,speed)
		print(speed)
	if is_on_wall() and Input.is_action_pressed("jump") and Input.is_action_pressed("dash") and Input.is_action_pressed("move_forward"):
			gravity_direction = Vector3.UP * speed 
func _input(event):
