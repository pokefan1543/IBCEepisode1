...
func _physics_process(delta):
	...
	if imageNum == 1:
		...
		$TextureRect.rect_scale.x = 0.5
		$TextureRect.rect_scale.y = 0.5
		$TextureRect.texture = KleoFinal
	if imageNum == 0:
		$ImageTitle.text = "Kleopatra's Design (Original)"
		$Author.text = "By: Noah Lanier"
		$TextureRect.rect_scale.x = 0.179
		$TextureRect.rect_scale.y = 0.179
		$TextureRect.texture = KleoOriginal
	if imageNum == 2:
		$ImageTitle.text = "Kleopatra's Sprite (Original)"
		$Author.text = "By: Claire Houser"
		$TextureRect.rect_scale.x = 0.43
		$TextureRect.rect_scale.y = 0.43
		$TextureRect.texture = KleoSprite1
	if imageNum == 3:
		$ImageTitle.text = "Kleopatra's Sprite (Final)"
		$Author.text = "By: Claire Houser"
		$TextureRect.rect_scale.x = 0.5
		$TextureRect.rect_scale.y = 0.5
