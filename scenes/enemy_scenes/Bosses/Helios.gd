...
func _physics_process(delta):
	if shoot == true:
		$SpotLightF.show()
		$Node3D/Helios/AnimationPlayer.play("default")
	...
	if enemy_following == true and backwards == false:
		set_velocity(Vector3(min(target.position.x - self.position.x, enemy_max_speed), gravity_direction_and_speed, min(target.position.z - self.position.z, enemy_max_speed)))
...
func _process(delta):
	match state:
		...
		ALERT:
			...
			if enemy_following2:
				...
				self.position.y = target.position.y + 100
	...
	if enemyhealth <= 0 and death == false:
		...
		self.add_sibling(player, key)
		gravity_direction_and_speed = 0
		enemy_max_speed = 0
		enemy_following = false
		death = true
		speed = 0
		$Node3D/Helios.queue_free()
...
func _on_strikeTimer_timeout():
	if attacktoggle == true:
		...
		enemy_following = true
		$SpotLightD.hide()
		self.position.z = -371.362
		self.position.y = 28.387
		self.position.x = 5.455
