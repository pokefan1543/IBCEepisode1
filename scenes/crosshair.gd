...
func _process(delta):
		if mouse_mov != null:
			if mouse_mov > sway_threshold:
				$Face2.rect_position = $Face2.rect_position.linear_interpolate(sway_left, sway_lerp * delta)
			if mouse_mov < sway_threshold:
				$Face2.rect_position = $Face2.rect_position.linear_interpolate(sway_right, sway_lerp * delta)
			else:
				$Face2.rect_position = $Face2.rect_position.linear_interpolate(sway_normal, sway_lerp * delta)
		if mouse_mov2 != null:
			if mouse_mov2 > sway_threshold:
				$Face2.rect_position = $Face2.rect_position.linear_interpolate(sway_up, sway_lerp * delta)
			if mouse_mov2 < sway_threshold:
				$Face2.rect_position = $Face2.rect_position.linear_interpolate(sway_down, sway_lerp * delta)
			else:
				$Face2.rect_position = $Face2.rect_position.linear_interpolate(sway_normal, sway_lerp * delta)
