...
onready var camera = $CollisionShape/Head/Camera
onready var hand = $CollisionShape/Head/Camera/hand
onready var anim_player = $CollisionShape/Head/Camera/hand/AnimationPlayer
onready var raycast = $CollisionShape/Head/Camera/hand/Buster/RayCast
onready var Chaospawnhurt = $ChaosPawnHurt
onready var text_timer = $text_timer
onready var coll = $CollisionShape
onready var ray = $CollisionShape/Head/Camera/hand/Buster/RayCast
onready var PulseRifle = $CollisionShape/Head/Camera/hand/M9PulseRifleHR
onready var AxelBuster = $CollisionShape/Head/Camera/hand/Buster
onready var pulseball = preload("res://scenes/PulseBall.tscn")
onready var tazerball = preload("res://scenes/Tazerball.tscn")
onready var aimcast2 = $CollisionShape/Head/Camera/hand/Stormcloud/RayCast
onready var pulsesound = $CollisionShape/Head/Camera/hand/pulsefire
onready var ray_container = $CollisionShape/Head/Camera/hand/ShotgunRayContainer
onready var shotgunmodel = $CollisionShape/Head/Camera/hand/shotgun
onready var stormcloud = $CollisionShape/Head/Camera/hand/Stormcloud
onready var flash = $CollisionShape/Head/Camera/hand/Stormcloud/stormflash
onready var stormmuzzle1 = $CollisionShape/Head/Camera/hand/muzzlestorm
#---------------------------------------------------------------------------
func fire_shotgun():
				...
				if not Input.is_action_pressed("fire2"):
					if Input.is_action_just_pressed("fire") and shotgun_fire == true:
						...
						if counter == 3 and not Input.is_action_pressed("fire"):
							$CollisionShape/Head/Camera/hand/shotgun/shotgun_timer.start()
						$muzzletimer.start(0.1)
						$CollisionShape/Head/Camera/hand/shotgun/shotgun.play(0.0001)
						$CollisionShape/Head/Camera/hand/shotgun/muzzleflash.show()
						anim_player.play("shotgunfire")
						$CollisionShape/Head/Camera/hand/shotgun/shotgun_timer.start()
						shotgun_fire = false
			...
			if Globals.shells > 0 and stormfire == true:
				if Input.is_action_just_pressed("fire"):
					...
					if aimcast2.is_colliding():
						...
						t.shoot = true
					$muzzletimer.start(0.1)
					$CollisionShape/Head/Camera/hand/Stormcloud/stormflash.show()
					$CollisionShape/Head/Camera/hand/tazerfire.play(0.001)
					stormfire = false
					$CollisionShape/Head/Camera/hand/Stormcloud/stormtimer.start()
		...
		if current_weapon == 6:
			if Globals.boltammo > 0:
				...
				if Input.is_action_just_released("fire"):
					...
					$CollisionShape/Head/Camera/hand/railgun/laser/Scaler.show()
					if $CollisionShape/Head/Camera/hand/railgun/laser/RayCast.is_colliding():
						#maybe use $Head/Camera/hand/railgun/RayCast instead
						var target = $CollisionShape/Head/Camera/hand/railgun/laser/RayCast.get_collider()
					...
					#railgunfire = false\
					raildamage = 0
					$CollisionShape/Head/Camera/hand/Buster/muzzleflash.show()
					$CollisionShape/Head/Camera/hand/railgun/railtimer.start()
					$CollisionShape/Head/Camera/hand/railgun/railgunfire.play(0.001)
func weapon_select():
	...
	if current_weapon == 6:
		$CollisionShape/Head/Camera/hand/railgun.visible = true
	else:
		$CollisionShape/Head/Camera/hand/railgun.visible = false
	if current_weapon == 2:
...
func fire():
	if Input.is_action_pressed("fire") and M9_fire == true:
		if Globals.pulseammo > 0:
			if current_weapon == 4:
				Globals.pulseammo -= 1
				if $Head/Camera/hand/M9PulseRifleHR/RayCast.is_colliding():
					var b = pulseball.instance()
					$Head/Camera/hand/M9PulseRifleHR/muzzle.add_child(b)
					b.look_at($Head/Camera/hand/M9PulseRifleHR/RayCast.get_collision_point(), Vector3.UP)
					b.shoot = true
	...
	if Input.is_action_just_pressed("fire") and Globals.upgrade == false:
		if current_weapon == 1:
			...
			$CollisionShape/Head/Camera/hand/Buster/burstTimer.start()
func _ready():
	randomize()
		...
		if highjump == true:
			if Input.is_action_just_pressed("dash"):
				...
				while dashnum != 100 and speed != 100 and not is_on_wall() and in_air == true:
					...
					_Camera.fov = lerp(_Camera.fov, fview["DASH"], ADS_LERP * delta)
					$dash.play()
					move_and_slide_with_snap(movement, snap, Vector3.DOWN)
				if is_on_floor():
					in_air = false
					if Input.is_action_just_pressed("jump"):
						speed = 70
...
func _on_muzzletimer_timeout():
	soundnum = 0
	$CollisionShape/Head/Camera/hand/Buster/muzzleflash.hide()
	$CollisionShape/Head/Camera/hand/shotgun/muzzleflash.hide()
	$CollisionShape/Head/Camera/hand/Stormcloud/stormflash.hide()
	$CollisionShape/Head/Camera/hand/railgun/laser/Scaler.hide()
	$CollisionShape/Head/Camera/hand/M9PulseRifleHR/muzzleflash.hide()
...
func _on_burstTimer_timeout():
	if current_weapon == 1:
		if burstnum != 5:
			...
			$CollisionShape/Head/Camera/hand/Buster/muzzleflash/firesound.play()
		else:
			$CollisionShape/Head/Camera/hand/Buster/burstTimer.stop()
	else:
		$CollisionShape/Head/Camera/hand/Buster/burstTimer.stop()
...
func _on_enter_body_entered(body):
	if enter == true:
		get_tree().change_scene("res://scenes/real_levels/E2M2part2.tscn")
...
func _on_enter4_body_entered(body):
	if enter3 == true:
		get_tree().change_scene("res://scenes/real_levels/E2M4part2.tscn")
