...
func _physics_process(_delta):
	...
	if enemy_following == true:
		move_and_slide(Vector3(min(player.translation.x - self.translation.x, enemy_max_speed), 0,  min(player.translation.z - self.translation.z, enemy_max_speed)), up_vector3)
func _process(_delta):
	match state:
		...
		ALERT:
			...
			rotate_x(deg2rad(-eyes.rotation.x * TURN_SPEED))
