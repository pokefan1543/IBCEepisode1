...
func _enter_tree():
	...
	_palette = PaletteControl.instantiate()
	_palette.connect("pattern_selected", Callable(self, "_on_Palette_pattern_selected"))
	_palette.connect("pattern_added", Callable(self, "_on_Palette_pattern_added"))
	_palette.connect("pattern_removed", Callable(self, "_on_Palette_pattern_removed"))
	_palette.hide()
	...
	_error_dialog.custom_minimum_size = Vector2(300, 200)
	_error_dialog.hide()
		...
		if action == ACTION_PAINT:
			...
			if not hit.is_empty():
				...
				if hit.collider == null or not (hit_instance_root.get_parent() is Scatter3D):
					...
					if not too_close:
						var instance = _pattern.instantiate()
...
func _show_error(msg):
	...
	_error_dialog.popup_centered_clamped()
