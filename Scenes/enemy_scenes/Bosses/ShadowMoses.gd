...
func _on_shoottimer_timeout():
	...
	if $Buster/RayCast3D.is_colliding():
		var hit = $Buster/RayCast3D.get_collider()
		...
		if target.health != 0 and target.health != -1:
			Shootsound.play()
	if $Buster2/RayCast3D.is_colliding():
		var hit = $Buster2/RayCast3D.get_collider()
		if hit.is_in_group("player"):
			...
			if target.health == -1:
				...
				$ALERTSOUND.stop()
				print("I need to stop")	
	if $Buster3/RayCast3D.is_colliding():
		var hit = $Buster3/RayCast3D.get_collider()
		if hit.is_in_group("player"):
			if target.health != 0 or target.health == -1:
				target.health -= damage
				playerhurtsound.play(0.0001)
			...
			if target.health == -1:
				...
				$ALERTSOUND.stop()
				print("I need to stop")	
	if $Buster4/RayCast3D.is_colliding():
		var hit = $Buster4/RayCast3D.get_collider()
		if hit.is_in_group("player"):
			if target.health != 0 or target.health == -1:
				target.health -= damage
				playerhurtsound.play(0.0001)
