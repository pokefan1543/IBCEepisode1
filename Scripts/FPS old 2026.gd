...
#func _handle_air_physics(delta) -> void:#TEST removed for upgrade #Movement physics now handled by state machine
#	self.velocity.y -= gravity * delta
#	self.velocity.x = wish_dir.x * speed
#	self.velocity.z = wish_dir.z * speed

#func _handle_ground_physics(delta) -> void:#TEST removed for upgrade #Movement physics now handled by state machine
#	self.velocity.x = wish_dir.x * speed
#	self.velocity.z = wish_dir.z * speed 

func _physics_process(delta):
	#TEST removed for upgrade #Velocity and movement controls now handled by state machine
	#var input_dir := Input.get_vector("left", "right", "up", "down").normalized()
	#wish_dir = (head.global_transform.basis * Vector3(input_dir.x, 0, input_dir.y)).normalized()
	#if is_on_floor():
	...
	#	self.velocity.y = jump
	controls(delta) #FIXME is this the proper place for this function call?
	move_and_slide()
